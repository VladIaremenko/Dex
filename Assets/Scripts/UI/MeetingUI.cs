...
public class MeetingUI : NetworkBehaviour
{
	...
	{
		if (GameManager.State.Current != GameState.EGameState.Meeting) return;
	...
	public void Server_SetTimer(Tick tick)
	{
		...
		int tickDiff = Runner.Simulation.Tick.Raw - tick.Raw;
	...
	void Server_EndMeeting()
	{
		...
		GameManager.State.Server_DelaySetState(GameState.EGameState.VoteResults, 1);
	}
