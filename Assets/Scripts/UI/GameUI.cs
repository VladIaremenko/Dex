...
public class GameUI : MonoBehaviour
{
	...
	public void InitTaskUI()
	{
		...
		foreach(TaskBase task in GameManager.Instance.Tasks().Select(ts => ts.taskUI.task).Distinct())
		{
		...
		{
			if (GameManager.Instance.HasTask(ts))
			{
	...
	public void UpdateTaskUI()
	{
		...
		{
			byte numTotal = GameManager.Instance.taskAmounts[taskItem.Task];
			int count = numTotal - tasks.Count(ts => ts.taskUI.task == taskItem.Task);
			taskItem.SetCount(count, numTotal);
			//if (tasks.Contains(taskUI.Task) == false)
			//	taskUI.Complete();
		}
